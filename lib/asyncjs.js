...
(function(name, context) {
    ...
    if (!ArrayPrototype.indexOf && Document.all && !Document.addEventListener) {
    ...
    if (Window.postMessage
        ...
        try {
            ...
            blob = new Blob([redirectMessage], { type: "text/javascript" });
        } catch (e) {
            var _blob = new BlobBuilder();
            _blob.append(redirectMessage);
            blob = _blob.getBlob("text/javascript");
        }
        ...
        thread = function(fn) { setTimeout(fn, 0); };
    }
    ...
    function resolveCallback() {
        ...
        // try again later
        if (self.digest()) {
            setTimeout(factory(resolveCallback, self), 50 / 3);
            return;
        }
